...
from cdrive_cleaner.executors import DirectFileDeleteExecutor, WindowsAdvancedExecutor
...
class MainWindow(QMainWindow):
    ...
    def _quick_clean(self) -> QWidget:
        ...
        self.quick_table.setHorizontalHeaderLabels(("选择", "类别", "风险", "预计空间"))
        self.quick_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
    ...
    def _analysis(self) -> QWidget:
        page, layout = QWidget(), QVBoxLayout()
        buttons = QHBoxLayout()
        self.analysis_button = QPushButton("分析 C 盘")
    ...
    def _refresh_disk(self) -> None:
        ...
        self.disk_label.setText(
            f"C 盘总容量 {_size(disk.total)}\n已使用 {_size(disk.used)}  ·  剩余 {_size(disk.free)}"
        )
    ...
    def _confirm_clean(self) -> None:
        ...
        self._run(
            lambda: service.execute(plan, volume=self._folders.system_drive, dry_run=False),
            self._clean_done,
        )
    ...
    def _clean_done(self, receipt: object) -> None:
        ...
        self.quick_status.setText(f"处理 {_size(processed)}，实际可用空间增加 {_size(observed)}")
        self.scan_button.setEnabled(True)
        self._refresh_disk()
        self._refresh_history()
    ...
    def _start_analysis(self) -> None:
        ...
        self.analysis_output.setPlainText("正在分析 C 盘……")
        self._run(
            lambda: StorageAnalyzer(top_n=25).analyze(
                self._folders.system_drive, token=self._token
            ),
            self._analysis_done,
        )
    ...
    def _analysis_done(self, value: object) -> None:
        ...
        lines.extend(
            f"{_size(item.logical_bytes):>10}  {item.path}" for item in value.top_directories
        )
        lines.extend(("", "最大文件："))
        lines.extend(f"{_size(item.logical_bytes):>10}  {item.path}" for item in value.top_files)
